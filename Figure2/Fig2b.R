plot.it <- function(ph, main) {
  ...
  xlab=""
  if (main == "LS") {
      xlab <- "Evolutionary Constraint"
  }
  plot(x, a$treat.prop, xlim=c(0,4), ylim=c(0, 0.0035), col=ccol, pch=cpch, main=main,
       xlab=xlab, ylab="", bty="n", xaxt="n")
...
phenos <- list(list("fa", "FA"),
               list("ls", "LS"),
               list("fn", "FN"))

postscript(file="panelb.ps", width=3.5, height=1.0, paper="special", pointsize=5, horizontal = FALSE)
par(mar=c(6,2,2,2))
...
cpch = 21
tpch = 19
par(mfrow=c(1,5), mar=c(4,2,2,2), lwd=0.6)
...
axis(1, at=1:3, labels=c("FA", "LS", "FN"), las=2)
