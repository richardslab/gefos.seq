...
treatment.fdr <- fdrtool.cutoff(m.d$pv[ m.d$treat == 1 ])
message("FDR Controls")
control.fdr <- fdrtool.cutoff(m.d$pv[ m.d$treat == 0 ])

out.file = paste(gerp.threshold, ".Rdata", sep="", collapse="")

sm.out <- summary(m.out)
save(gerp.threshold, treatment.fdr, control.fdr, sm.out, file=out.file)
